                ...
                <form action="{{ route('post.update', $post->id) }}" method="POST">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            id="title" name="title" type="text" value="{{ $post->title }}" required autofocus autocomplete="" />
                    </div>
                    <div>
                        <label for="body">Body</label>
                        <textarea
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline mt-1 block"
                            id="body" name="body" required>{{ $post->body }}</textarea>
                    </div>
                    <div class="flex items-center justify-end gap-4">
                        <button class="px-4 py-2 bg-blue-400 rounded-md hover:bg-blue-500" type="submit">Update</button>
                    </div>
                </form>
